...
const posts = ref(postStore.editorPosts.data);
const user = ref(userStore.user);
const currentPage = ref(1);

const isAdmin = computed(() => userStore.role === "admin");
const totalPages = ref(userStore.editorPosts?.last_page);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value) {
    currentPage.value++;
    postStore.fetchEditorPosts(currentPage.value, user.value.id);
  }
};
...
onBeforeMount(() => {
  postStore.fetchEditorPosts(1, user.value.id);
});

watch(
  () => postStore.editorPosts,
  (newValue) => {
